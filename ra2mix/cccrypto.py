...
def get_decryption_block_sizing(file_count: int) -> tuple[int, int]:
    ...
    remaining_index_len = index_len - SIZE_OF_FILE_COUNT
    padding_size = BYTE_SIZE - remaining_index_len % BYTE_SIZE
...
def decrypt_mix_header(mix_data: bytes, key: bytes) -> tuple[int, int, bytes]:
    ...
    decrypted_block = cipher.decrypt_block(
        mix_data[header_start : header_start + BLOCK_SIZE]
    )

    (file_count, data_size) = struct.unpack(
        "=H I", decrypted_block[: SIZE_OF_FILE_COUNT + SIZE_OF_DATA_SIZE]
    )
    ...
    data_decrypted = b"".join(
        cipher.decrypt_ecb(
            mix_data[
                header_start + BLOCK_SIZE : header_start + BLOCK_SIZE + decrypt_size
            ]
        )
    )

    data_size_in_first_block = BYTE_SIZE - SIZE_OF_FILE_COUNT - SIZE_OF_DATA_SIZE
    index_decrypted = (
        decrypted_block[-data_size_in_first_block:] + data_decrypted[:-padding_size]
    )
